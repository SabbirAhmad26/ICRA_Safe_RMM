...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space, for_carla=False):
        self.episode_length = args.episode_length
        # if we need to truncate the first X steps from replay buffer
        # if args.env_name == 'CARLA' and args.fs_step > 0:
        #     self.episode_length -= args.fs_step
        ...
        assert not self._use_centralized_V
        '''
        ...
        if False:
            pass
            # print("Buffer for carla")
            # obs_shape = obs_space
            # share_obs_shape = share_obs_space
            # #act_shape = len(act_space)
            # self.available_actions = np.ones((self.episode_length + 1, self.n_rollout_threads, act_space[0]), dtype=np.float32)
        else:
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        if len(self.share_obs.shape) > 3:
            ...
            next_obs = self.obs[1:].transpose(1, 0, 2, 3, 4).reshape(-1, *self.obs.shape[2:])
        else:
        ...
        # x.transpose(1,0,2).reshape(-1, *x.shape[2:])
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        '''
            episode_length = 300, n_thread = 1
            batch_size = 300, data_chunk_length = 30(for example)
            data_chunks = 300 // 30 = 10
            ...
            num_mini_batch = 3
            => mini_batch_size = 3
            sampler = [rand[0:3], rand[3:6], rand[6:9]]
