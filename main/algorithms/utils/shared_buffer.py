...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        '''
            When using centralized Value function, 
                Value:
                    the input state is global state,
                    the output is estimated value of total return of all agents
                Advantage is 

            When using decentralized value function,
                Value:
                    the input state is local state
                    the output is estimated value of total return of individual agent
        '''
        ...
        '''
            ...
            In the buffer:
                ...
                obs: [300, 1, 3, 128]
                rnn_states, rnn_states_critic: [300, 1, 3, recurrent_N, hidden_size]
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        ...
        """
        # if using centralized critic, each rewards should be the total reward of all agents
        step_wise_rewards = self.rewards
        if self._use_centralized_V:
            self.total_rewards = np.sum(self.rewards, axis=2, keepdims=True).repeat(repeats=self.rewards.shape[2], axis=2)
            step_wise_rewards = self.total_rewards
        ...
        if self._use_proper_time_limits:
            if self._use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        delta = step_wise_rewards[step] + self.gamma * value_normalizer.denormalize(
                        ...
                        delta = step_wise_rewards[step] + self.gamma * self.value_preds[step + 1] * self.masks[step + 1] - \
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        self.returns[step] = (self.returns[step + 1] * self.gamma * self.masks[step + 1] + step_wise_rewards[
            ...
            if self._use_gae:
                ...
                self.returns[-1] = next_value
                for step in reversed(range(self.rewards.shape[0])):
                    self.returns[step] = self.returns[step + 1] * self.gamma * self.masks[step + 1] + step_wise_rewards[step]
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        # print("generator:", self.share_obs.shape)
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        """
        # for worst-case algorithm, we need to output 3 more outputs
        episode_length, n_rollout_threads, num_agents = self.rewards.shape[0:3]
        batch_size = n_rollout_threads * episode_length * num_agents
        ...
        rnn_states_critic = self.rnn_states_critic[:-1].transpose(1, 2, 0, 3, 4).\
        ...
        for indices in sampler:
            ...
            next_masks_batch = np.stack(next_masks_batch)
