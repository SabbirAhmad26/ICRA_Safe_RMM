...
from main.envs.carla.control import mpc_exec_highway, mpc_exec_crossing, high_level_lane_changing, crossing_high_level_controller
import re
...
class CARLARunner(Runner):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                for vid, cav in self.envs.CAV_dict.items():
                    ...
                    if self.all_args.run_rule_based_benchmark:
                        command = agents_dict[vid]["recomended_command"] 
                    actions_env[vid] = self.command_2_action[agents_dict[vid]["recomended_command"]]
                    ...
                    try:
                        ...
                        if self.all_args.scenario_name == "crossing":
                            if self.all_args.run_rule_based_benchmark:
                                status, action, slack_vars = crossing_high_level_controller(agents_dict[vid], acc_ref, states, preceding_cars_state, conflicting_cars_state_degree1, conflicting_cars_state_degree2, vid, vid_select, current_lane_index,non_cavs_lane_ids, MPs,conflicting_cars_state_degree3)
                            else:
                ...
                print_stepwise_msg(self.algorithm_name, cavs_list, actions_env, actions_env, all_car_info_dict, \
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                for vid, cav in self.envs.CAV_dict.items():
                    ...
                    command = agents_dict[vid]["recomended_command"] 
                    actions_env[vid] = self.command_2_action[command]
            ...
            if episode % self.log_interval == 0:
                ...
                if self.env_name == 'CARLA':
                    ...
                    episode_dict = group_episode_results(all_rewards, all_flow_rewards, all_dest_rewards, \
                        cols_rwds, safe_rwds, enco_rwds, self.all_args.flow_reward_coef, self.all_args.gamma)
                    episode_dict['switch'] = self.envs.switch
                    episode_dict['any_collision'] = self.envs.done_collision
    ...
    # def print_stepwise_msg(self, actions, true_actions, all_car_info_dict, episode, step, rewards, mean_rwd_items, rwd_items):
    #     print('=========================================================')
