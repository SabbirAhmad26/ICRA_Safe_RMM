...
def collision_handler(event, col_queue):
    ...
    print("Collide: Vehicle {:d} with vehicle: {:d}; intensity: {:.2f}".\
...
class CarEnv:
    def __init__(self, args, num_CAVs = 1):
        ...
        # RL params
        self._policies = None
        self.brief_state = True
        if self.brief_state:
            self.base_state_length = 7
            self.max_num_car = 6
        else:
            self.base_state_length = 16
            self.max_num_car = 10
        self.observation_space = dict.fromkeys(range(num_CAVs), [self.base_state_length*self.max_num_car])
        self.share_observation_space = dict.fromkeys(range(num_CAVs), [self.base_state_length*self.max_num_car*num_CAVs])
    ...
    def get_agents_dict(self):
        ...
        for vid, cav in self.CAV_dict.items():
            ...
            temp_cav = {'position': dict(x = 0, y = 0), 'theta': None, 'speed': None, "command": "lane_keeping", 'initial_position': dict(x = 0, y =0),
                             "previous_command": "lane_keeping", "reference_lane": None, "MPs" : [], "done":False,
    ...
    def reset(self, scenario):
        ...
        if self.map_name == 'Town06':
            ...
            if self.scenario == 'highway':
                ...
                if self.test_only:
                    config_dict = config["Highway-Test"]
                else:
                    config_dict = config["Highway-baseline-1"]
                ...
                if self.test_only:
                    config_dict = config["Crossing-Test"]
                else:
                    config_dict = config["Crossing-baseline-1"]

                self.spawn_locs, self.destinations, self.switch = spawn_from_config_new(config_dict, self.num_CAVs, self.num_UCVs, \
                            self.scenario, self.episode_length, self.cav_force_straight_step)

            elif self.scenario == 'merging':
        ...
        if self.scenario == 'crossing':
            #self.hazard_throttle = config_dict["HAZV_base_throttle"] + np.random.rand()*0.01
            #self.ncav_throttle = config_dict["NCAV_base_throttle"] + np.random.rand()*0.01
            
            # UCV target v: 9-11; HZV target v: 7.5 - 12.5
            ...
            self.ncav_throttle = np.random.rand()*0.1 + 0.80
            if not self.normal_behave:
                self.hzv_hardbrake_start = random.randrange(self.ucv_force_straight_step, self.ucv_force_straight_step+30)
        ...
        for i in range(self.num_Vs):
            ...
            if i < self.num_CAVs:
                ...
                initial_velocity = carla.Vector3D(x=5, y=0, z=0)  # 10 m/s in the x-direction
                ...
                if self.scenario == 'crossing':
                    ...
                    print("NCAV {:d} base throttle: {:.2f}; Initial Spawn ({:.2f}, {:.2f})".\
                        format(vehicle.id, self.ncav_throttle, transform.location.x, transform.location.y))
    ...
    def step(self,agents_dict, action_dict=None, step_n=-1):
        """
                ...
                for vid, ncav in self.ncav_vehicles.items():
                    ...
                    # set ncav with constant throttle value
                        ...
                        if self.normal_behave or (vid not in self.hazard_ncav_ids) or \
                            ...
                            rand_target_speed = np.random.rand()*2 + 10.
                            if ncav_speed >= rand_target_speed:
                                ncav_target_throttle = search_throttle_value(self.poly_dict, 0., rand_target_speed - 0.5)
                            else:
                                ncav_target_throttle = search_throttle_value(self.poly_dict, 0., rand_target_speed + 0.5)
                            ncav_control = carla.VehicleControl(throttle=ncav_target_throttle + ncav_acc_revise, steer=ncav_steer)
                        else:
                            if self.hzv_hardbrake_start <= step_n < self.hzv_hardbrake_start + 25:
                                haz_ttl_revise = np.random.rand()*0.1 - 0.5
                                ncav_control = carla.VehicleControl(throttle=0., brake=self.ncav_throttle+haz_ttl_revise, steer=ncav_steer)
                            else:
                                # base brake 0.6 - 0.9
                        ...
                        if self.normal_behave:
                            ...
                            if step_n>= 206:
                                ...
                                if ncav_speed >= ncav_target_speed:
                                    ncav_target_throttle = search_throttle_value(self.poly_dict, 0., ncav_target_speed - 0.5)
                                else:
                                    ncav_target_throttle = search_throttle_value(self.poly_dict, 0., ncav_target_speed + 0.5)
        ...
        return obs_dict, done, all_car_info_dict, (vels_rwds, dest_rwds, col_penalties, sact_rwds), safe_action_dict, force_continue
